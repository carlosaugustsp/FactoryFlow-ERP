...
import { Plus, Package, Database, Trash2, Save, AlertTriangle, Edit3, Shield, Upload, List, X } from 'lucide-react';
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
        ...
        const newItems: Product[] = data.map((row: any) => ({
            ...
            sku: row['SKU'] || 'SKU-TEMP',
            reference: row['Referencia'] || row['Ref'] || '',
            category: row['Categoria'] || 'Outros',
            price: Number(row['Preco'] || row['Price'] || 0),
            stockRaw: activeTab === 'MATERIALS' ? Number(row['Estoque'] || 0) : 0,
            stockFinished: activeTab === 'PRODUCTS' ? Number(row['Estoque'] || 0) : 0,
            type: activeTab === 'MATERIALS' ? 'MATERIA_PRIMA' : 'PRODUTO_FINAL',
            unitOfMeasure: (row['Unidade'] || 'UNIDADE').toUpperCase() as UnitOfMeasure
        }));
